...
if __name__ == "__main__":
    ...
    authors_names_03_19_cleaned = [name.strip().replace(" /", "") for name in authors_names_03_19 if name.strip()]
    print(authors_names_03_19_cleaned)

    repo_descriptions_03_19 = tree.xpath('/html/body//article/p/text()')
    repo_descriptions_03_19_cleaned = [name.strip() for name in repo_descriptions_03_19 if name.strip()]
    print(repo_descriptions_03_19_cleaned)

    languages_03_19 = tree.xpath('/html/body//article/div[@class="f6 color-fg-muted mt-2"]/span/span[2]/text()')
    languages_03_19_cleaned = [name.strip() for name in languages_03_19 if name.strip()]
    print(languages_03_19_cleaned)

    total_stars_03_19 = tree.xpath('/html/body//article/div[@class="f6 color-fg-muted mt-2"]/a[1]/text()')
    total_stars_03_19_cleaned = [name.strip() for name in total_stars_03_19 if name.strip()]
    print(total_stars_03_19_cleaned)

    total_forks_03_19 = tree.xpath('/html/body//article/div[@class="f6 color-fg-muted mt-2"]/a[2]/text()')
    total_forks_03_19_cleaned = [name.strip() for name in total_forks_03_19 if name.strip()]
    print(total_forks_03_19_cleaned)

    built_by_03_19 = tree.xpath('/html/body//article/div[@class="f6 color-fg-muted mt-2"]/span[2]//a/img/@src')
    built_by_03_19_cleaned = [name.strip() for name in built_by_03_19 if name.strip()]
    print(built_by_03_19_cleaned)

def save_data_to_excel(*args, file_name='Github_Trending_Data.xlsx'):
    ...
    for data_row in zip(*args, ['2024_03_19' for _ in range(len(args[0]))]):
        ws.append(data_row)
...
if __name__ == "__main__":
    save_data_to_excel(repo_names_03_19_cleaned, authors_names_03_19_cleaned, repo_descriptions_03_19_cleaned, languages_03_19_cleaned, total_stars_03_19_cleaned, total_forks_03_19_cleaned, built_by_03_19_cleaned)
